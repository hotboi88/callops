// Auth screen — shown when there's no Supabase session (or a session
// exists but the user isn't in the user_invites/profiles allowlist).
//
// Two states this component handles:
//   1. signed-out → "Sign in with Google" button
//   2. signed-in-but-no-profile → "Access denied" + sign out
//
// Renders nothing if CONFIG.USE_SUPABASE is false (skeleton stays open
// for local dev / demo).
const { useState: useStateAuth, useEffect: useEffectAuth } = React;

function AuthScreen({ mode }) {
  const [busy, setBusy] = useStateAuth(false);
  const [err, setErr] = useStateAuth(null);

  const signIn = async () => {
    setBusy(true); setErr(null);
    try { await window.SB.auth.signInWithGoogle(); }
    catch (e) { setErr(e.message || String(e)); setBusy(false); }
  };
  const signOut = async () => {
    setBusy(true);
    try { await window.SB.auth.signOut(); window.location.reload(); }
    catch (e) { setErr(e.message || String(e)); setBusy(false); }
  };

  if (mode === "denied") {
    return (
      <div className="auth-screen">
        <div className="auth-card">
          <div className="auth-brand"><div className="brand-mark">CO</div> <span>CallOps</span></div>
          <h1>Access denied</h1>
          <p className="auth-sub">
            You're signed in, but this email isn't on the invite list.
            Ask an admin to invite you, then sign back in.
          </p>
          <button className="btn btn-block" onClick={signOut} disabled={busy}>
            {busy ? "…" : "Sign out"}
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="auth-screen">
      <div className="auth-card">
        <div className="auth-brand"><div className="brand-mark">CO</div> <span>CallOps</span></div>
        <h1>Sign in</h1>
        <p className="auth-sub">CallOps is invite-only. Sign in with the Google account your admin invited.</p>
        <button className="btn btn-primary btn-block btn-google" onClick={signIn} disabled={busy}>
          {busy ? "…" : (
            <>
              <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09 0-.73.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Sign in with Google
            </>
          )}
        </button>
        {err && <p className="auth-err">{err}</p>}
      </div>
    </div>
  );
}

window.AuthScreen = AuthScreen;
